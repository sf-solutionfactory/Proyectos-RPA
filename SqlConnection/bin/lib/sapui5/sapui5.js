...
ctx.apiui5 = {
   ...
   */
    ...
    var timeout = item.parent.getPageWaitReadyTimeout();
    ctx.noNotify = true;
    var res = item.execScript('SAPUI5MultiInputAddToken', data, timeout);
    ctx.notifyAction('SAPUI5MultiInputAddToken', res, desc);
    return res;
  },
	/**
   * Call the api ui5 control method.
    ...
    if (oControl) {
	    try {
				...
				if(res.length===0){
						oControl["setValue"]();
						data.type = "sap.m.Token";
						var value = SAPUI5Convert(data);
					  res = oControl["addToken"].apply(oControl, [value]);
				}
		}catch(oErr) {         
    }	
    }
...
ctx.customTypes.SAPUI5.multiinput = function(item) {
  ...
  item.parent.customMethods.SAPUI5MultiInputRemoveToken =  ctx.customTypes.SAPUI5.findCodeToInject('SAPUI5MultiInputRemoveToken');
	...
	item.parent.customMethods.SAPUI5MultiInputAddToken =  ctx.customTypes.SAPUI5.findCodeToInject('SAPUI5MultiInputAddToken');
  /**
   * Sets the value of item.
   * @description
   *  <wrap help> //Example://</wrap>
