...
GLOBAL.scenario({ newWorkflow: function(ev, sc) {
	...
	sc.step(GLOBAL.steps.Start_Saplogon, GLOBAL.steps.pHome_management);
...
//   Step: Start_Saplogon
// ----------------------------------------------------------------
GLOBAL.step({ Start_Saplogon: function(ev, sc, st) {
	var rootData = sc.data;
	ctx.workflow('openSAP', '15f5540a-07ff-4b51-b391-89e0d9d6c397') ;
...
GLOBAL.step({ pHome_management: function(ev, sc, st) {
	var rootData = sc.data;
	ctx.workflow('openSAP', 'dd62eede-7979-4a28-9ece-e1c4dc5c63ee') ;
...
GLOBAL.step({ pSAPLogin_management: function(ev, sc, st) {
	var rootData = sc.data;
	ctx.workflow('openSAP', '56bebd08-75b1-4619-b0e7-1d103f459894') ;
	// Wait until the Page loads
...
GLOBAL.step({ pSAPEasyAccess_manage: function(ev, sc, st) {
	var rootData = sc.data;
	ctx.workflow('openSAP', 'dbbe70e5-1de3-4a83-8529-8eedc5bcecc8') ;
	// Wait until the Page loads
