...
GLOBAL.scenario( {
	cargaVentas: function (ev, sc) {
		...
		sc.onTimeout(100000, function (sc, st) {
			sc.endScenario();
...
GLOBAL.step( {
	getFilename: function (ev, sc, st) {
		...
		var date = new Date();
		rootData.datetime = date.getFullYear() + date.getMonth() + date.getDate();
		var mes = date.getMonth() + 1;
		if (mes < 10) {
			mes = "0" + mes;
		}
		rootData.fechaActual = date.getDate() + "/" + mes + "/" + date.getFullYear();
...
// ----------------------------------------------------------------
GLOBAL.step( {
	Initialize_Excel: function (ev, sc, st) {
		var rootData = sc.data;
...
// ----------------------------------------------------------------
GLOBAL.step( {
	Open_existing_Excel_f: function (ev, sc, st) {
		var rootData = sc.data;
...
// ----------------------------------------------------------------
GLOBAL.step( {
	pWindowMicrosoftEx_ma: function (ev, sc, st) {
		var rootData = sc.data;
...
// ----------------------------------------------------------------
GLOBAL.step( {
	pWindowMacro_manageme: function (ev, sc, st) {
		var rootData = sc.data;
...
// ----------------------------------------------------------------
GLOBAL.step( {
	pWindowReporteador_ma: function (ev, sc, st) {
		var rootData = sc.data;
...
// ----------------------------------------------------------------
GLOBAL.step( {
	pWindowMessage_manage: function (ev, sc, st) {
		var rootData = sc.data;
...
// ----------------------------------------------------------------
GLOBAL.step( {
	Activate_worksheet: function (ev, sc, st) {
		var rootData = sc.data;
...
// ----------------------------------------------------------------
GLOBAL.step( {
	Leer_lista: function (ev, sc, st) {
		var rootData = sc.data;
		ctx.workflow('cargaVentas', '20c1fd0f-b8e9-4122-8d1e-33b2f5b65a59');
		// Describe functionality to be implemented in JavaScript later in the project.
		rootData.length = ctx.excel.sheet.getLastRow('A1') -1;
		rootData.length = rootData.length + "";
...
// ----------------------------------------------------------------
GLOBAL.step( {
	Custom: function (ev, sc, st) {
		var rootData = sc.data;
		ctx.workflow('cargaVentas', '1542d61d-223c-4f1d-b786-2c54571cf5e4');
		// Describe functionality to be implemented in JavaScript later in the project.
		//var count = ctx.excel.sheet.getLastRow('A1') -1;
		var count = 10;
		for (var index = 1; index < count; index++) {
			var txt = "";
			for (var j = 0; j < 35; j++) {
				if (rootData.Items[index][j] == undefined) {
					rootData.Items[index][j] = "";
				}
				if (j == 20 || j == 21) {
					txt = txt + "0002004768\t";
				}else {
					txt = txt + rootData.Items[index][j] + "\t";
				}
			}
...
// ----------------------------------------------------------------
GLOBAL.step( {
	Save_Excel_file: function (ev, sc, st) {
		var rootData = sc.data;
...
// ----------------------------------------------------------------
GLOBAL.step( {
	Close_Excel_file: function (ev, sc, st) {
		var rootData = sc.data;
