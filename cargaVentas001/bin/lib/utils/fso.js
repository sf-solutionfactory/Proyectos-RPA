...
ctx.options.fso = {
 ...
 ctx.fso.file.loadXml( xmlfile, 'XML' );
     ...
     * @param       {boolean} [selectFile] need to be to 'true' to select a file
     * @return      {string} Selected path or filename
     * @path        ctx.fso.file.openDialog
     */
     _file.openDialog = function (title, options, rootFolder, hwnd, selectFile) {
       var res = '';
       ...
       try {
         if (selectFile) {
           var ie = new ActiveXObject("InternetExplorer.Application");
					 ie.visible = false;
           ie.Navigate(rootFolder)
					 ie.Document.write("<html><body><input id=\"fileselect\", name=\"fileselect\", type=\"file\"></body></html>");
           var fs = ie.Document.all.fileselect;
           fs.focus();
           fs.click();
           ctx.sleep(500);
           return fs.value;
           ...
           hwnd = hwnd || 0;
           if (!hwnd && ctx.currentEvent && ctx.currentEvent.page && ctx.currentEvent.name !== "UNLOAD" && ctx.currentEvent.name !=="END" ) {
     ...
     */
     _file.write = function(filename, content, encoding, overwrite)
     {
       ...
       try {
		 ...
		 {
			throw new Error(e.error.KO, 'content \'' + content + '\' is integer');
		 }
     ...
     */
     _file.zip = function(files, zipFile, fileOperation) {
       ...
       try{
		 ...
		 // PLO - SAPMLIPA-11582 Ctx.Diagnostic dont work anymore (2080304976, Internal Incident)
		 // return file rather than _file, which is undefined
		 return file ;
       } catch (ex) {
...
// specific events
GLOBAL.addEvent( { evFileBrowsing:'' });
// no display in debugger
GLOBAL.events.evFileBrowsing.internal = true;