...
GLOBAL.labels.set({
  ...
  error: { 
    ...
    UndefinedObject: { _comment:"The object is not defined", _type:"XFLD", en:"The object is not defined" },
		NoAttendedLicence: { _comment: 'No license to run attended jobs.', _type: 'XFLD', en: 'No license to run attended jobs.' },
    ConcurrentQuotaReached: { _comment: 'Maximum number of concurrent attended execution reached. Cannot add job.', _type: 'XFLD', en: 'Maximum number of concurrent attended execution reached. Cannot add job.' },
    NoUnattendedLicence: { _comment: 'No license to run unattended jobs.', _type: 'XFLD', en: 'No license to run unattended jobs.' }
...
//	};
	...
	/** @type {boolean} */ var _hasAskingSwitch = false;
	...
	/**
		 * @ignore
     	 * Description 
		 * @method hasAskingSwitch
		 * @path ctx.m2m.hasAskingSwitch
		 * @return {boolean}
		 */
		self.hasAskingSwitch = function () {
			return _hasAskingSwitch;
		}

		/**
		 * @ignore
     	* Description 
		 * @method setAskingSwitching
		 * @path ctx.m2m.setAskingSwitching
		 * @param {boolean} askingSwitch
		 */
		self.setAskingSwitching = function (askingSwitch) {
			_hasAskingSwitch = askingSwitch;
		}
		...
		if ((ev.data) && ('string' === typeof ev.data)) {
			...
			if (msgContent.State) {
				switch (msgContent.State) {
					...
					case 'ASK_SWITCH':
						if  ('undefined' !== typeof systray) {
							systray.displayLoadingSpinner();
						}
						ctx.m2m.setAskingSwitching(true);
						break;
			...
			if (msgContent.Error){
					switch (msgContent.Error) {
						case 'BAD_ANSWER_RESOURCE_INFOS':
							if(ctx.m2m.hasAskingSwitch())
							{
								ctx.registry.del('Software\\SAP\\Intelligent RPA\\Desktop Agent\\LastAttendedProjectEnvironment', ctx.enums.registry.root.CurrentUser);
								ctx.registry.del('Software\\SAP\\Intelligent RPA\\Desktop Agent\\LastAttendedProjectVersion', ctx.enums.registry.root.CurrentUser);
								ctx.registry.del('Software\\SAP\\Intelligent RPA\\Desktop Agent\\LastAttendedProject', ctx.enums.registry.root.CurrentUser);
			...
			if (msgContent.Error != undefined) {
				systray.removeLoadingSpinner();//SAPMLIPA-11416: If Agent returns any error, remove the loading spinner
			}	
