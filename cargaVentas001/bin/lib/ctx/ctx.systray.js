...
ctx.systrayClass = function (name, parentProcess) {
  ...
  * @property {string} name  */ this.name = name;
  ...
  var _bPopupWasNotReadyForEnvUpdate = false;
  var _bPopupWasNotReadyForDefaultModeUpdate = false;
  var _bPopupWasNotReadyForAgentStatusUpdate = false;
  ...
  var _postUpdateUIDynamicElements = function () {
    ...
    var res= false;
      ...
      if (_bPopupWasNotReadyForEnvUpdate) { //SAPMLIPA-10100 : in case popup does not exist at sendClientInfos reception
        _systray.updateEnvironmentList();
      }
      if (_bPopupWasNotReadyForAgentStatusUpdate) {
        _systray.updateAgentStatus();
      }
      if (_bPopupWasNotReadyForDefaultModeUpdate) {
        _systray.setDefaultMode();
      }
  ...
  */
  this.display = function () {
		...
		if (POPUPS && ('undefined' === typeof POPUPS.Systray)) {
			POPUPS.events['START'].on(function (ev) {
...
systray.showBalloon( ctx.options.projectLabel, "Ready for testing...", e.systray.iconType.Warning, 10000 );
 ...
 /**@method      onAgentStatusUpdate
  * @summary     Function used to receive updates on agent status
  * @description
  * @path        ctx.systrayClass.onAgentStatusUpdate
  * @param       {function(e.agentStatus, string, string)} callback Callback to be called on status change
  */
  this.onAgentStatusUpdate = function (callback) {
    if (callback && ('function' === typeof callback)) {
			_agentStatusCallbacks.push(callback);
		}
	}

 /**@method      updateAgentStatus
  * @summary     Update agent status in Systray [internal usage]
  * @description
  * @ignore
  ...
  this.updateAgentStatus = function (agentStatus, label, description, skip) {
    var res = false;
      ...
      if (description !== undefined)
			...
			if (_popup.exist()) {
        _bPopupWasNotReadyForAgentStatusUpdate = false;
        _popup.execScript('updateAgentStatus', _agentStatus, _statusLabel, _statusDescription);
      } else {
        _bPopupWasNotReadyForAgentStatusUpdate = true;
      }
			ctx.lockNoNotify = false;
			ctx.each(_agentStatusCallbacks, function(index, callback) {
		    if (callback && ('function' === typeof callback)) {
					try {
						callback(agentStatus, label, description);
					} catch (ex) {	}				
				}
			});
 ...
 this.updateEnvironmentList = function (environmentList, studioMode) {
  ...
  if (_newMode && _popup) {
    ...
    if (_popup.exist()) {
      _bPopupWasNotReadyForEnvUpdate = false;
      _popup.execScript('updateEnvironmentList', _environmentList, currentEnvUid);
    } else {
      _bPopupWasNotReadyForEnvUpdate = true;
    }
    res = true;
  } else if (!_newMode) {
...
this.setDefaultMode = function (bDefaultMode) {
  ...
  ctx.lockNoNotify = true;
  if (_newMode && _popup) {
    if ('undefined' !== typeof bDefaultMode) {
			_defaultMode = bDefaultMode;
		}
    if (_popup.exist()) {
      _bPopupWasNotReadyForDefaultModeUpdate = false;
      _popup.execScript('setIsDefaultModeAndUpdateUI', _defaultMode);
    } else {
      _bPopupWasNotReadyForDefaultModeUpdate = true;
    }
    res = true;
  }
  ctx.lockNoNotify = false;
...
/**
  * @method      removeLoadingSpinner
  * @ignore
  * @description
  * Removes the loading spinner
  * @path        ctx.systrayClass.removeLoadingSpinner
  * @return      {boolean} Return value
  */
this.removeLoadingSpinner = function () {
  var res = false;
  ctx.lockNoNotify = true;
  if (_newMode && _popup) {
    if (_popup.exist()) {
      _popup.execScript('_removeLoadingSpinner()');
    }
    res = true;
  }
  ctx.lockNoNotify = false;
...
/**
  * @method      displayLoadingSpinner
  * @ignore
  * @description
  * Displays the loading spinner
  * @path        ctx.systrayClass.displayLoadingSpinner
  * @return      {boolean} Return value
  */
this.displayLoadingSpinner = function () {
  ...
  if (_newMode && _popup) {
    if (_popup.exist()) {
      _popup.execScript('_displayLoadingSpinner()');
    }
    res = true;
  }
  ctx.lockNoNotify = false;
