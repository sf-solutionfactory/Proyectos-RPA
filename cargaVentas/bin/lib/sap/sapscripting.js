...
ctx.SAPScripting = (function() {
	...
	/** Used to identify the caller of LockRefresh or UnlockRefresh APIs in the backend */
	/** Not to be modified */
	var REFRESH_CALLER = 'SDK';
	...
	var GuiFrameWindowObj = function(itemOrPage, type){
		...
		*/
		this.keyStroke = function (command) {
			...
			return '' === ctx.SAPScripting.executeSAPGuiFunction(desc, type, 'keyStroke',command,'','','');
		};
    ...
    */
	...
	var GuiComboBoxObj = function (itemOrPage, type) {
		/**
		* @description Returns the entries in ComboBox.
		* __Ex.:__
		<code javascript>
			SAPLogon750.oPage.oComboBox.getList();
		</code>
		* @method getList
		...
		*/
		this.getList = function () {
			var desc = itemOrPage.getObjectDescriptor();
			ctx.notifyAction('getList', '', desc);
			return ctx.actionApp(desc, 'getList', 'get_ENTRIES', desc.itemFullName, '', '', '', '');
		};
	
		/**
		* @description Returns the selected key in ComboBox.
		* __Ex.:__
		<code javascript>
			SAPLogon750.oPage.oComboBox.getKey();
		</code>
		* @method getKey
		* @path ctx.item.getKey
		* @return {string}
		*/
		this.getKey = function () {
			var desc = itemOrPage.getObjectDescriptor();
			ctx.notifyAction('getKey', '', desc);
			return ctx.SAPScripting.executeSAPGuiFunction(desc, type, 'getKey', '', '', '', '');
		};

		/**
		* @description Sets the key as selected item in ComboBox.
		* __Ex.:__
		<code javascript>
			SAPLogon750.oPage.oComboBox.setKey(key);
		</code>
		* @method setKey
		* @path ctx.item.setKey
		*/
		this.setKey = function (value) {
			var desc = itemOrPage.getObjectDescriptor();
			ctx.notifyAction('setKey', '', desc);
			return ctx.SAPScripting.executeSAPGuiFunction(desc, type, 'setKey', value, '', '', '');
		};

		/**
		* @description Returns the selected value in ComboBox.
		* __Ex.:__
		<code javascript>
			SAPLogon750.oPage.oComboBox.getValue();
		</code>
		* @method getValue
		* @path ctx.item.getValue
		* @return {string}
		*/
		this.getValue = function () {
			var desc = itemOrPage.getObjectDescriptor();
			ctx.notifyAction('getValue', '', desc);
			return ctx.SAPScripting.executeSAPGuiFunction(desc, type, 'getValue', '', '', '', '');
		};

		/**
		* @description Sets the value as selected item in ComboBox.
		* __Ex.:__
		<code javascript>
			SAPLogon750.oPage.oComboBox.setValue();
		</code>
		* @method setValue
		* @path ctx.item.setValue
		*/
		this.setValue = function (value) {
			var desc = itemOrPage.getObjectDescriptor();
			ctx.notifyAction('setValue', '', desc);
			return ctx.SAPScripting.executeSAPGuiFunction(desc, type, 'setValue', value, '', '', '');
		};
		...
		* __Ex.:__
		<code javascript>
			SAPLogon750.oPage.oComboBox.selected();
		</code>
		* @method selected
		* @path ctx.item.selected
		*/
		this.selected = new commonFunctions(itemOrPage, type).get;

		/**
		* @description Sets the value as selected item.
		* __Ex.:__
		<code javascript>
			SAPLogon750.oPage.oComboBox.select();
		</code>
		* @method select
		* @path ctx.item.select
		*/
		this.select = new commonFunctions(itemOrPage, type).set;
	...
	var GuiTableControlObj = function(itemOrPage, type){
		...
		*/
		this.getVisibleRows = function () {
			...
			ctx.actionApp(desc, 'Unlock Refresh', 'UNLOCKREFRESH', desc.itemFullName, REFRESH_CALLER, '', '', '');
	...
	var GuiCtrlTreeObj = function(itemOrPage, type){
		...
		*/
		this.selectContextMenuItem = function (command) {
			...
			if (sSelected) {
				...
				res = '' === ctx.SAPScripting.executeSAPGuiFunction(desc, type, 'selectContextMenuItemByText', command, '','','');
			}
		...
		*/
		this.selectContextMenuItemByText = function (nodeKey, itemText, columnPos) {
			...
			//Lock and Unlock Refresh is added to make sure that Page does not UNLOAD until RELEASESESSION is executed.
			ctx.actionApp(desc, 'Lock Refresh', 'LOCKREFRESH', desc.itemFullName, REFRESH_CALLER, '', '', '');
			ctx.actionApp(desc, 'Hold Session', 'HOLDSESSION', desc.itemFullName, '', '', '', '');
			ctx.noNotify = true;
			var res = false;
			...
			try {
				if (nodeKey) {
					...
					if (canExecute) {
						res = '' === ctx.SAPScripting.executeSAPGuiFunction(desc, type, 'selectContextMenuItemByText', itemText, '','','');
					}
				}
			}
			catch (err)	{
		...
		*/
		this.selectContextMenuItemByPosition = function (nodeKey, itemPosition, columnPos) {
			...
			try {
				if (nodeKey) {
					...
					if (canExecute) {
						res = '' === ctx.SAPScripting.executeSAPGuiFunction(desc, type, 'selectContextMenuItemByPosition', itemPosition, '','','');
					}
				}
			}
			catch (err) {
		...
		/**
		* Select a Context menu item using item ID
		* @description Available on Tree View Control
		* __Ex.:__
		<code javascript>
		SAPLogon760.pSAPEasyAccess.oSAPTableTreeControl.selectContextMenuItemByID("Logistics", "Open folder", 0)
		</code>
		* @method selectContextMenuItemByID
		* @path ctx.item.selectContextMenuItemByID
		* @param {string} nodeKey
		* @param {string} ID Context Menu Item ID
		* @param {number} columnPos Position of the Column from left (By default 0)
		* @return {boolean} result 
		*/
		this.selectContextMenuItemByID = function (nodeKey, itemID, columnPos) {
			var desc = itemOrPage.getObjectDescriptor();
			ctx.notifyAction('selectContextMenuItemByID', '', desc);
			//Lock and Unlock Refresh is added to make sure that Page does not UNLOAD until RELEASESESSION is executed.
			ctx.actionApp(desc, 'Lock Refresh', 'LOCKREFRESH', desc.itemFullName, REFRESH_CALLER, '', '', '');
			ctx.actionApp(desc, 'Hold Session', 'HOLDSESSION', desc.itemFullName, '', '', '', '');
			ctx.noNotify = true;
			...
			try {
				if (nodeKey) {
					...
					if (canExecute) {
						res = '' === ctx.SAPScripting.executeSAPGuiFunction(desc, type, 'selectContextMenuItem', itemID, '','','');
					}
				}
			}
			catch (err) {
	...
	*/
	var GuiCtrlGridViewObj =  function(itemOrPage, type){
		...
		*/
		...
		/**
		* Select a Context menu item using item text
		* @description Available on Gui Grid View
		* __Ex.:__
		<code javascript>
		SAPLogon760.pSAPEasyAccess.oPane.selectContextMenuItemByText("Logistics")
		</code>
		* @method selectContextMenuItemByText
		* @path ctx.item.selectContextMenuItemByText
		* @param {string} itemText Context Menu Item text
		* @return {boolean} result 
		*/
		this.selectContextMenuItemByText = function (itemText) {
			var desc = itemOrPage.getObjectDescriptor();
			ctx.notifyAction('selectContextMenuItemByText', '', desc);
			ctx.noNotify = true;
			//Lock and Unlock Refresh is added to make sure that Page does not UNLOAD until RELEASESESSION is executed.
			...
			ctx.actionApp(desc, 'Hold Session', 'HOLDSESSION', desc.itemFullName, '', '', '', '');
			try {
				ctx.SAPScripting.executeSAPGuiFunction(desc, type, 'contextMenu', '', '','','');
		...
		/**
		* Select a Context menu item using item position
		* @description Available on Gui Grid View
		* __Ex.:__
		<code javascript>
		SAPLogon760.pSAPEasyAccess.oPane.selectContextMenuItemByPosition(0)
		</code>
		* @method selectContextMenuItemByPosition
		* @path ctx.item.selectContextMenuItemByPosition
		* @param {string} itemPos Context Menu Item Position
		* @return {boolean} result
		*/
		this.selectContextMenuItemByPosition = function (itemPos) {
			var desc = itemOrPage.getObjectDescriptor();
			ctx.notifyAction('selectContextMenuItemByPosition', '', desc);
			ctx.noNotify = true;
			//Lock and Unlock Refresh is added to make sure that Page does not UNLOAD until RELEASESESSION is executed.
		...
		* __Ex.:__
		<code javascript>
		SAPLogon760.pSAPEasyAccess.oPane.selectContextMenuItemByID("&FIND")
		</code>
		* @method selectContextMenuItemByID
		* @path ctx.item.selectContextMenuItemByID
		* @param {string} itemID Context Menu Item ID
		* @return {boolean} result 
		*/
		this.selectContextMenuItemByID = function (itemID) {
			var desc = itemOrPage.getObjectDescriptor();
			ctx.notifyAction('selectContextMenuItemByID', '', desc);
			ctx.noNotify = true;
			//Lock and Unlock Refresh is added to make sure that Page does not UNLOAD until RELEASESESSION is executed.
		...
		*/
		this.getVisibleRows = function () {
			...
			ctx.actionApp(desc, 'Unlock Refresh for Get Rows', 'UNLOCKREFRESH', desc.itemFullName, REFRESH_CALLER, '', '', '');
			return rows;
		};
		...
		*/
		this.setCell = function (value, row, col) {
			...
			return '' === ctx.SAPScripting.executeSAPGuiFunction(desc, type, 'setCell', row, col, value, '');
		};
	...
	var GUIGridViewPaneObj = function(itemOrPage, type){
		...
		this.selectContextMenuItemByText = gridViewObj.selectContextMenuItemByText;
		this.selectContextMenuItemByPosition = gridViewObj.selectContextMenuItemByPosition;
		this.selectContextMenuItemByID = gridViewObj.selectContextMenuItemByID;
	};
