  ...
  function _createMultipartData( formData, boundary ) {
    ...
    ctx.each(formData, function(id, formData) {
      if (formData && formData.name ) {
        ...
        if (formData.file) {
          formData.file = ctx.wscript.shell.expandEnvString(formData.file);
...
var text = ctx.clipboard.get();
      ...
      */
      get : function () {
        var desc = ctx.getDescriptor();
        var str = ctx.verbExec(desc, 'ctx.clipboard.get', 'SETVALUE', {
            ZoneCtx : '_Work0_',
            Value : '_Clipboard_'
          });
          return str && str.replace(/&#xA&#xD/g, '\r\n');
      },
      /**
